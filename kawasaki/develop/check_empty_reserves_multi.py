...
def search_empty_reserves_from_datesearch(cfg, cookies, form_data, datetime_list, reserves_list):
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        for _datetime in datetime_list:
            ...
            futures.append(executor.submit(request_empty_reserve, _datetime, cfg, headers, cookies, params))
            # デバッグ用としてhtmlファイルとして保存する
        ...
        for future in as_completed(futures):
            #print(type(future))
            #print(dir(future))
            _datetime = future.result()[0]
            res = future.result()[1]
            #print(f'{future}')
            #print(f'{_datetime} : {res}')
...
# 指定日と検索結果を返す
@reserve_tools.elapsed_time
def request_empty_reserve(*args, **kwargs):
    """
    並行処理のために外に出す
    """
    datetime = args[0]
    ...
    params = args[4]
    try:
        res = requests.post(cfg['search_url'], headers=headers, cookies=cookies, data=params, timeout=(3.0, 7.5))
        ...
        while _retry < max_retry:
            if res.status_code != 200:
                ...
                res = requests.post(cfg['search_url'], headers=headers, cookies=cookies, data=params, timeout=(3.0, 7.5))
                _retry += 1
                print(f'request faild count: {_retry}')
            else:
                print(f'success post request: {datetime}')
                break
        else:

            
            print(f'faild post request: {datetime}')
    except Timeout:
        print(f'request timeout: {datetme}')
    else:
        print(f'{res.headers}')
        print(dir(res))
        # デバッグ用としてhtmlファイルとして保存する
...
def analyze_html(cfg, cookies, datetime, res, reserves_list) :
    ...
    print(f'starting analyze response: {_date} {_time}')
    ...
    print(f'finished analyze response: {_date} {_time}')
    return None
