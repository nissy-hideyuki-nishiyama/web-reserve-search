# 変更点
# - asyncioとaiohttpによる非同期処理
# - 利用日時ページ以降を非同期処理
# うまくいっていない部分
# - cookieとformdataについて全て同じものを使っているため、システムエラーが頻発し、空き予約検索が正しくできない
...
async def post_request_from_datesearch(cfg, cookies, form_data, datetime_list, reserves_list, coro, http_req_num, limit=1):
    """
    並列処理数を制限しながらHTTPリソースを取得するコルーチン
    """
    # 非同期タスクリストの初期化
...
## 利用日時ページに検索データを入力して検索する
@reserve_tools.elapsed_time
async def search_empty_reserves_from_emptystate(cfg, cookies, datetime, form_data, res, reserves_list):
    """
    利用日時と利用目的、地域を入力して空き予約を検索する
    """
    # リダイレクトされているか確認する
...
def prepare_proc():
    ...
    return ( cfg, cookies, form_data, datetime_list)

# 空き予約を検索する
@reserve_tools.elapsed_time
def main(cfg, cookies, form_data, datetime_list, coro, limit=2):
    """
    空き予約検索のメインルーチン
    """
    # 空き予約の辞書の初期化
    ...
    ( responses, http_req_num ) = loop.run_until_complete(post_request_from_datesearch(cfg, cookies, form_data, datetime_list, reserves_list, coro, http_req_num, limit))
...
if __name__ == '__main__':
    ...
    async def coroutine(cfg, cookies, headers, form_data, response):
        return cookies, headers, response.status, response.reason, response.headers, await response.text()
    ...
    # 事前準備
    (cfg, cookies, form_data, datetime_list) = prepare_proc()
    ...
    ( responses, http_req_num ) = main(cfg=cfg, cookies=cookies, form_data=form_data, datetime_list=datetime_list, coro=coroutine, limit=2)
    ...
    for res in responses:
        # HTMLボディ本体はres[3]
        print(f'http status / reason: {res[2]} / {res[3]}')
        print(f'response headers: {res[4]}')
        if res[2] == 200:
            reserves_list = analyze_html_02(cfg, cookies, res[5], reserves_list)
            # デバッグ用としてhtmlファイルとして保存する
            #_datetime_string = str(datetime[0]) + str(datetime[1]).zfill(2) + str(datetime[2]).zfill(2) + str(datetime[3]).zfill(2) + str(datetime[4]).zfill(2)
            #_file_name = f'result_{_datetime_string}.html'
            #print(_file_name)
            #_file = reserve_tools.save_html_to_filename(res[3], _file_name)
