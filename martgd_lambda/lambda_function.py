...
# POSTリクエストを実行する
def do_post_request(cookies, form_data, pre_url, url, logger=None):
    """
...
def do_search_empty_reserve(cfg, cookies, response, input_data, court_empty_reserves_list, logger=None):
    ...
    for _court, _in_data in input_data.items():
        ...
        _html = do_post_request(cookies, form_data, url, url, logger=logger)
        #logger.debug(_html.text)
...
def analize_html(court, html, logger=None):
    ...
    logger.info(empty_reserves_list)
