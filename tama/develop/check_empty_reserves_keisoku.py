#
# check_empty=reserves_keisoku.pyの目的
# - HTTPリクエスト回数と各HTTPリクエスト時間およびその合計値を取得するため
# check_empty_reservesとの差分
# - HTTPリクエスト回数を取得するため、http_req変数を追加する
# - HTTPリクエスト実行時間を取得するため、デコレータの@reserve_tools.elapsed_timeを追加する
# - HTTPリクエストが失敗したときの再試行処理の追加とタイムアウトの例外を追加する
#
# モジュールの読み込み
## HTMLクローラー関連 
...
def get_empty_reserves(cfg, date_list, reserves_list, cookies, http_req_num):
    ...
    for _day in date_list:
        ...
        for param_string in cfg['search_params']:
            ...
            http_req_num += 1
            # 空きコートのリンクを取得する
            court_link_list = get_court(res[1], cfg, _day)
                ...
                for _exclude_court in cfg['exclude_courts']:
                    if _exclude_court in atag['href']:
                        ...
                        if _match_count >= _exclude_court_count:
                            ...
                            if f'{day}' !=  f'{_link_day}':
                                print(f'## Different Input: {day} / Link: {_link_day}')
