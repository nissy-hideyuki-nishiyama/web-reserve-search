...
def get_empty_reserves(cfg, date_list, reserves_list, court_link_list, cookies, http_req_num, th_lock_list, th_lock_urls):
    """
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for _day in date_list:
            ...
            for param_string in cfg['search_params']:
                ...
                http_req_num += 1
                _days_resp[f'{_entity}'] = executor.submit(get_request_retry, _day, _entity, search_url, headers_day, cookies, cfg, th_lock_urls)
...
def get_request_retry(*args, **kwargs):
    ...
    th_lock_urls = args[6]
    try:
        _res[f'{_entity}'] = requests.get(_search_url, headers=headers, cookies=cookies, timeout=(6.0, 15.0))
        # ステータスコード200以外は再試行を3回する
        max_retry = 3
        _retry = 0
        while _retry < max_retry:
            if _res[f'{_entity}'].status_code != 200:
                sleep(1)
                _res[f'{_entity}'] = requests.post(_search_url, headers=headers, cookies=cookies, timeout=(6.0, 15.0))
                _retry += 1
                print(f'get request faild count: {_retry}')
            else:
                #print(f'success get request: {_entity}')
                get_court(_res[f'{_entity}'], cfg, _day, th_lock_urls)
                break
        else:
...
def main():
    ...
    (reserves_list, http_req_num ) = get_empty_reserves(cfg, date_list, reserves_list, court_link_list, cookies, http_req_num, th_lock_list, th_lock_urls)
