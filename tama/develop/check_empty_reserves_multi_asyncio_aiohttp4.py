#
# check_empty_reserves_multi_asyncio_aiohttp4.pyの目的
# - HTTPリクエストの関数側のコートの空き予約時間帯を処理するコードを移動する
#
# モジュールの読み込み
## HTMLクローラー関連
...
async def get_empty_reserves(cfg, date_list, reserves_list, cookies, http_req_num, th_lock_reserves, conn_limit):
    """
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for _day in date_list:
                ...
                for param_string in cfg['search_params']:
                    ...
                    # 指定年月日を指定して検索リクエストを送信し、空きコートリンクを取得する
                    #res = requests.get(search_url, headers=headers_day, cookies=cookies)
                    http_req_num += 1
                    _day_resp = await aio_get_request_retry(session, _day, _entity, search_url, headers_day, cookies)
                    ...
                    # 空きコートリンクを取得する
                    court_link_list = get_court(_day_resp[2], cfg, _day_resp[0])
                    ...
                    #print(type(court_link_list))
                    #print(dir(court_link_list))
                    #print(_days_resp)
                    ...
                    for link in court_link_list:
                        ...
                        http_req_num += 1
                        _court_resp = await aio_get_request_retry(session, _day, _entity_string, search_url, headers_court, cookies)
                        # 空き予約を取得する
                        get_empty_time(_court_resp[2], cfg, _court_resp[0], th_lock_reserves)
...
# 非同期IOのHTTPリクエストをする
@reserve_tools.elapsed_time
async def aio_get_request_retry_old(*args, **kwargs):
    """
    並行処理のために外に出して、ステータスコード200以外なら再試行する
    """
    session = args[0]
...
def get_court(response, cfg, day):
    """
...
def main():
    ...
    ( reserves_list, http_req_num ) = loop.run_until_complete(get_empty_reserves(cfg, date_list, reserves_list, cookies, http_req_num, th_lock_reserves, conn_limit))
