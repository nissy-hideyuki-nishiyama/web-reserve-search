#
# check_empty_reserves_multi_asyncio_aiohttp2.pyの目的
# - asyncioの最初の習作
# - Responseから日付を取得するように変更する
# - HTTPリクエスト実行時間の計測を追加
# 
# モジュールの読み込み
## HTMLクローラー関連
...
async def get_empty_reserves(cfg, date_list, reserves_list, court_link_list, cookies, http_req_num, th_lock_list, th_lock_url, conn, loop):
    """
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for _day in date_list:
                ...
                for param_string in cfg['search_params']:
                    ...
                    #_day_resp = asyncio.ensure_future(aio_get_request_retry(session, _day, _entity, search_url, headers_day, cookies))
                    _day_resp = loop.create_task(aio_get_request_retry(session, _day, _entity, search_url, headers_day, cookies))
                    ...
                    #( _res, _day ) = ( _day_resp.result()[0], _day_resp.result()[1] )
            ...
            #exit()
        #loop.run_until_complete(asyncio.gather(*_days_resp))
            for _resp in await asyncio.gather(*_days_resp):
                #print(f'_resp info {_resp[1]} start: =================')
                #print(type(_resp))
                #print(dir(_resp))
                #print(_resp)
                ( _res, _day ) = ( _resp[2], _resp[0] )
                get_court(_res, cfg, _day, th_lock_url)
                #print(f'_resp info {_resp[1]} end: =================')
...
def main():
    ...
    #(reserves_list, http_req_num ) = get_empty_reserves(cfg, date_list, reserves_list, court_link_list, cookies, http_req_num, th_lock_list, th_lock_url, conn)
    ( reserves_list, http_req_num ) = loop.run_until_complete(get_empty_reserves(cfg, date_list, reserves_list, court_link_list, cookies, http_req_num, th_lock_list, th_lock_url, conn, loop))
