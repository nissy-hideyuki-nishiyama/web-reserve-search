...
def get_empty_reserves(response, cfg, day, reserves_list):
    ...
    #    return None
    ## レスポンスオブジェクトをHTML化する## レスポンスオブジェクトをHTML化する
    html = str(response)
    soup = BeautifulSoup(html, features='html.parser')
    # コート名を取得する
...
@reserve_tools.elapsed_time
async def get_request_time(cfg, cookies, court_link_list, coro, http_req_num, limit=1):
    """
    並列処理数を制限しながらHTTPリソースを取得するコルーチン
    """
    tasks = []
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
        for _day, _link_list in court_link_list.items():
            for _link in _link_list:
                ...
                _entity = f'{_day}_{name}'
                # urlrリストオブジェクトを作成する
                url = [ _day, _entity, search_url, headers_court, cookies ]
                http_req_num += 1
                task = asyncio.ensure_future(bound_get_request_fetch(semaphore, url, session, coro))
                tasks.append(task)
        responses = await asyncio.gather(*tasks)
...
def main(urls, coro, limit=4):
    ...
    print(f'####################################')
    #exit()
    return results, http_req_num

# メイン２ルーチン
def main2(http_req_num, cfg, cookies, court_link_list, coro, limit=4):
    """
    メインルーチン2
    """
    # 実行時間を測定する
    ...
    # 空き予約検索を開始する
    ## 検索のためのリクエストオブジェクトを作成する
    ( results, http_req_num ) = loop.run_until_complete(get_request_time(cfg, cookies, court_link_list, coro, http_req_num, limit))
    # デバッグ用(HTTPリクエスト回数を表示する)
    print(f'HTTP リクエスト数: {http_req_num} 回数')
    # 実行時間を表示する
    elapsed_time = time.time() - start
...
if __name__ == '__main__':
    ...
    ( reserves_results, http_req_num ) = main2(http_req_num, cfg=cfg, cookies=cookies, court_link_list=court_link_list, coro=coroutine, limit=4)
