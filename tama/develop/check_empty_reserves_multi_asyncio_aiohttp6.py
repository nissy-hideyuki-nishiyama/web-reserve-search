#
# check_empty_reserves_multi_asyncio_aiohttp6.pyの目的
# - asyncioLockを２つ以上導入する
#
#
...
async def get_empty_courts(cfg, date_list, async_lock_urls, cookies, http_req_num, conn, loop):
    """
        ...
        #async with aiohttp.ClientSession() as session:
            ...
            for _day in date_list:
                ...
                for param_string in cfg['search_params']:
                    ...
                    _day_task = loop.create_task(aio_get_request_retry(session, _day, _entity, search_url, headers_day, cookies, cfg, async_lock_urls))
    ...
    async with session.get(_search_url, headers=headers, cookies=cookies) as res:
       async with lock:
            ...
            if f'/cu/ykr132241/app/ykr30000/ykr31101.aspx' in _search_url:
                ...
                get_court(cfg, _html, async_lock_urls)
            else:
...
def get_court(cfg, response, court_link_list):
    """
...
def main():
    ...
    ( court_htmls, http_req_num ) = loop.run_until_complete(get_empty_courts(cfg, date_list, court_link_list, cookies, http_req_num, conn, loop))
