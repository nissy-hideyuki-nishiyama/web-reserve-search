#
# check_empty_reserves_multi_asyncio_aiohttp5.pyの目的
# - asyncioLockを導入する
#
# モジュールの読み込み
## HTMLクローラー関連
...
async def get_empty_reserves(cfg, date_list, reserves_list, court_link_list, cookies, http_req_num, th_lock_reserves, async_lock_urls, conn, loop):
    """
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for _day in date_list:
                ...
                for param_string in cfg['search_params']:
                    ...
                    _day_resp = loop.create_task(aio_get_request_retry(session, _day, _entity, search_url, headers_day, cookies, cfg, async_lock_urls, th_lock_reserves))
            ...
            await asyncio.gather(*_days_resp)
...
#@reserve_tools.elapsed_time
#async def get_court(response, cfg, day, th_lock_urls):
...
def main():
    ...
    ( reserves_list, http_req_num ) = loop.run_until_complete(get_empty_reserves(cfg, date_list, reserves_list, court_link_list, cookies, http_req_num, th_lock_reserves, async_lock_urls, conn, loop))
