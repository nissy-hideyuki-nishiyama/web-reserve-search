...
- 上記のdict型リストを使って、予約処理を開始する
  ...
  - IDの既存の予約済みリストと件数(全体件数と翌月件数)を取得し、予約上限数を超えている場合は次のIDで予約処理をする
...
- 予約処理において、次を設定できる
  ...
  - 予約開放日(open_day、多摩市の場合は16日)を指定する。予約開放日によって挙動が変わる
    - 予約開放日前と以降では予約上限数と翌月予約上限数が変わる。それぞれ指定する
...
- reserved_limit_for_next_month: 翌月の予約上限数(予約開放日前)を指定する
- reserved_limit_for_next_month_after_open_day: 翌月の予約上限数(予約開放日以降)を指定する
